<script lang="ts">
  ...
  // Keeps the last image visible while Overlay's close transition plays,
  // since the caller nulls `image` the instant it closes the dialog.
  let displayImage: GalleryEntry | null = null;
  $: if (image) displayImage = image;
</script>

<Overlay
  open={Boolean(open && image)}
  {onClose}
  closeLabel={$t.galleryEditDialog.closeLabel}
  labelledBy="gallery-edit-dialog-title"
  describedBy="gallery-edit-dialog-subtitle"
  z={90}
    ...
    <div class="border-b border-stone-200 p-5 dark:border-zinc-800">
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          ...
          <p class="mt-3 truncate rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-600 dark:border-zinc-800 dark:bg-zinc-950/60 dark:text-zinc-400" title={displayImage.filename}>
            {$t.galleryEditDialog.imageLabel(displayImage.filename)}
          </p>
        </div>
