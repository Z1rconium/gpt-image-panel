...
const zh: Translation = {
  ...
  promptForm: {
    ...
    maskEditLabel: (label) => `编辑 ${label} 的局部重绘选区`,
    removeMask: '移除选区',
    planEdit: '规划编辑',
    planningEdit: '规划中...',
    edits: '编辑',
  ...
  maskEditor: {
    ...
    smooth: (px) => `边缘平滑 ${px}px`,
    smoothHint: '只补平凹口与细缝，不会删除已画区域',
    snap: '路径吸附',
    snapHint: '把矩形、椭圆与套索的路径点吸附到最近的图像边缘',
    snapRadius: (px) => `吸附 ${px}px`,
    snapGroupLabel: '路径吸附',
    regionGroupLabel: '区域处理',
    autoFill: '自动补全空缺',
    autoFillHint: '闭合选区内部的细缝、针孔与画框细条',
    autoFillStrength: '强度',
    strengthLight: '轻',
    strengthNormal: '标准',
    strengthStrong: '强',
    edgeSnap: '贴合物体边缘',
    edgeSnapHint: '把选区扩展到附近的强边缘上。这是启发式的：纹理复杂时请检查预览。',
    edgeSnapRange: (px) => `贴边范围 ${px}px`,
    grow: (px) => `边缘外扩 ${px}px`,
    growHint: '把最终选区向外扩张固定距离',
    coverageWithAuto: (pct, autoPct) => `编辑区域 ${pct}%（自动补全 +${autoPct}%）`,
    coveragePending: '…',
    autoFillLargeHint: '自动补全新增较多，请检查预览',
    coverageTooSmall: '编辑区域很小，确认不是误触？',
    coverageTooLarge: '几乎覆盖整图，可以考虑不使用蒙版',
    undo: '撤销',
    redo: '重做',
    invert: '反选',
    clear: '清空',
    uploadMask: '上传蒙版',
    uploadMaskHint: '支持 PNG、JPEG、WebP；透明蒙版以 alpha < 128 为编辑区，黑白蒙版以白色为编辑区',
    uploadBlackWhiteNotice: '已按黑白蒙版导入：白色为编辑区。可用反选切换。',
    uploadScalePrompt: (width, height, targetWidth, targetHeight) =>
      `蒙版为 ${width}x${height}，主图为 ${targetWidth}x${targetHeight}。按最近邻缩放后导入？`,
    uploadScale: '缩放导入',
    uploadChooseMode: '当前已有选区：导入的蒙版要替换它，还是与它合并？',
    uploadReplace: '替换选区',
    uploadMerge: '与现有合并',
    viewOverlay: '叠加',
    viewMaskOnly: '仅蒙版',
    ...
    sizeHint: (size) => `本次请求尺寸为 ${size}；蒙版始终与主图尺寸一致。开启回贴时按主图尺寸输出。`,
    shortcutsTitle: '键盘快捷键',
    shortcuts: 'B/E/R/O/L/H 切换工具 · [ ] 调整笔刷 · S 路径吸附 · G 自动补全空缺 · Shift 点击画直线、Shift 拖动约束正方形或圆 · 空格或中键拖动平移 · 滚轮缩放 · Alt 临时反选 · 0 适应画布 · Esc 取消当前拖动 · Ctrl/⌘+Z 撤销 · Ctrl/⌘+Shift+Z 或 Ctrl/⌘+Y 重做',
    importFailed: '无法读取该蒙版图片',
    busy: '正在处理蒙版...'
  },
  ...
  messages: {
    ...
    editRetryMaskMissing: '无法读取该任务保存的蒙版。可以重新编辑选区，或不带蒙版重试。',
    editRetryMaskMissingTitle: '无法恢复原任务蒙版',
    editRetryMaskMismatchTitle: '蒙版与当前主图不匹配',
    editRetryMaskMismatch: '原任务蒙版的尺寸与当前主图不一致，无法确认是同一张图。请重新编辑选区，或不带蒙版重试。',
    editRetryWithoutMask: '无蒙版重试',
    editRetryReeditMask: '重新编辑选区',
    editRetryMaskCleared: '该任务未使用蒙版，已忽略当前选区',
    editMaskApplied: (pct) => `选区已保存 · 可编辑区域 ${pct}%`,
    editMaskRestored: (pct) => `已恢复原任务选区 · 可编辑区域 ${pct}%`,
    editMaskRemoved: '已保存为无蒙版',
    editMaskDiscarded: '主图已变化，蒙版已清除',
    editMaskStale: '蒙版与当前主图不匹配，请重新打开蒙版编辑器并应用。',
    editMaskUploadNotPng: '蒙版必须是 PNG、JPEG 或 WebP',
