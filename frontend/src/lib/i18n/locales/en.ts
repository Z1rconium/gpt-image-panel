const en = {
  ...
  promptForm: {
    ...
    maskEditLabel: (label: string) => `Edit repaint area for ${label}`,
    removeMask: 'Remove area',
    planEdit: 'Plan edit',
  ...
  maskEditor: {
    title: (label: string) => `Repaint area · ${label}`,
    closeLabel: 'Close repaint editor',
    ...
    lasso: 'Lasso',
    pan: 'Move view',
    modeGroupLabel: 'Selection mode',
    addMode: 'Add',
    subtractMode: 'Subtract',
    brushSize: (px: number) => `Brush ${px}px`,
    smooth: (px: number) => `Edge smoothing ${px}px`,
    smoothHint: 'Only fills notches and seams; it never removes area you painted',
    snap: 'Path snap',
    snapHint: 'Pulls rectangle, ellipse and lasso path points onto the nearest image edge',
    snapRadius: (px: number) => `Snap ${px}px`,
    snapGroupLabel: 'Path snapping',
    regionGroupLabel: 'Region processing',
    autoFill: 'Fill gaps',
    autoFillHint: 'Closes seams, pinholes and border strips inside the selection',
    autoFillStrength: 'Strength',
    strengthLight: 'Light',
    strengthNormal: 'Standard',
    strengthStrong: 'Strong',
    edgeSnap: 'Follow object edges',
    edgeSnapHint: 'Grows the selection onto a nearby strong edge. Heuristic: check the preview on busy textures.',
    edgeSnapRange: (px: number) => `Edge reach ${px}px`,
    grow: (px: number) => `Grow edges ${px}px`,
    growHint: 'Dilates the finished selection by a fixed distance',
    coverageWithAuto: (pct: string, autoPct: string) =>
      `Edit area ${pct}% (auto-fill +${autoPct}%)`,
    coveragePending: '…',
    autoFillLargeHint: 'Auto-fill added a lot; check the preview before saving',
    coverageTooSmall: 'The edit area is very small. Was that intentional?',
    coverageTooLarge: 'Almost the whole image is selected. Consider editing without a mask.',
    undo: 'Undo',
    redo: 'Redo',
    invert: 'Invert',
    clear: 'Clear',
    uploadMask: 'Upload mask',
    uploadMaskHint: 'PNG, JPEG or WebP. Alpha below 128 or white in a black and white mask is editable.',
    uploadBlackWhiteNotice: 'Imported as a black and white mask: white is editable. Use Invert to switch.',
    uploadScalePrompt: (width: number, height: number, targetWidth: number, targetHeight: number) =>
      `Mask is ${width}x${height}; primary image is ${targetWidth}x${targetHeight}. Scale with nearest neighbor and import?`,
    uploadScale: 'Scale and import',
    uploadChooseMode: 'A selection already exists: replace it with the imported mask, or merge them?',
    uploadReplace: 'Replace selection',
    uploadMerge: 'Merge with existing',
    viewOverlay: 'Overlay',
    viewMaskOnly: 'Mask only',
    zoomIn: 'Zoom in',
    zoomOut: 'Zoom out',
    zoomLabel: 'Zoom view',
    zoom100: '100% (original pixels)',
    fitCanvas: 'Fit canvas',
    coverage: (pct: string) => `Edit area ${pct}%`,
    greenHint: 'Green marks the area to modify',
    apply: 'Save selection',
    saveNoMask: 'Save without mask',
    cancel: 'Cancel',
    noAreaMarked: 'Mark at least one region first',
    sizeHint: (size: string) => `The request size is ${size}; the mask always matches the primary image. Paste back outputs at primary image size.`,
    shortcutsTitle: 'Keyboard shortcuts',
    shortcuts: 'B/E/R/O/L/H tools · [ ] brush size · S path snap · G fill gaps · Shift-click brush line or Shift-drag square/circle · Space or middle-drag pan · wheel zoom · Alt inverts add/subtract · 0 fit canvas · Esc cancels a drag · Ctrl/⌘+Z undo · Ctrl/⌘+Shift+Z or Ctrl/⌘+Y redo',
    importFailed: 'The mask image could not be read',
    busy: 'Rendering mask...'
  },
  ...
  messages: {
    ...
    editRetryMaskMissing: 'The mask saved for this job could not be loaded. Re-edit the selection, or retry without a mask.',
    editRetryMaskMissingTitle: 'Cannot restore the original mask',
    editRetryMaskMismatchTitle: 'Mask does not match the current image',
    editRetryMaskMismatch: 'The original mask size differs from the current primary image, so it cannot be confirmed as the same picture. Re-edit the selection, or retry without a mask.',
    editRetryWithoutMask: 'Retry without mask',
    editRetryReeditMask: 'Re-edit selection',
    editRetryMaskCleared: 'This job used no mask; the current selection was ignored',
    editMaskApplied: (pct: string) => `Selection saved · ${pct}% editable`,
    editMaskRestored: (pct: string) => `Original task selection restored · ${pct}% editable`,
    editMaskRemoved: 'Saved as no mask',
    ...
    editMaskUploadNotPng: 'The mask must be PNG, JPEG or WebP',
